...
def best_first_search(start, end):
    visited = set()
    heap = [(distance(city_coordinates[start], city_coordinates[end]), start, [])]
    ...
    while heap:
        ...
        if current not in visited:
            ...
            for neighbor in adjacency_graph.get(current, []):
                if neighbor not in visited:
                    heapq.heappush(heap, (distance(city_coordinates[neighbor], city_coordinates[end]), neighbor, path + [current]))
    return None

# A* search
def astar_search(start, end):
    visited = set()
    heap = [(0, distance(city_coordinates[start], city_coordinates[end]), start, [])]
        ...
        if current not in visited:
            ...
            for neighbor in adjacency_graph.get(current, []):
                if neighbor not in visited:
                    ...
                    h = distance(city_coordinates[neighbor], city_coordinates[end])  # Heuristic cost
...
while True:
    ...
    if path:
        ...
        print("Time taken:", round(end_time - start_time, 8), "seconds")
    else:
